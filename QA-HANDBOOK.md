...
Если README — это витрина магазина, то этот файл — каталог с описанием каждого товара, инструкциями и кейсами применения.
...
6. [🎯 Сценарии собеседований](#сценарии-собеседований)
...
└── README.md                  ← 🏪 ВИТРИНА: коротко, ярко, для рекрутера
```
...
- json-server проще для портфолио (один npm пакет)
...
### Вопрос: «Зачем Docker с mock API?»

**Ответ (30 сек):**
> «Когда backend ещё не готов, я поднимаю json-server в Docker на основе OpenAPI спецификации. Пишу тесты на mock, потом переключаю BASE_URL на реальный стенд. Добавляю middleware для ошибок — 500, 429, 422 — которые настоящий API не воспроизведёт.»

### Вопрос: «Почему 3 тест-фреймворка?»

**Ответ:**
> «Это не дублирование — это разные уровни. Postman — для быстрой ручной проверки и демо заказчику. TypeScript + Jest — для CI/CD, data-driven тестов, JSON Schema валидации. k6 — для нагрузки тысяч пользователей. Каждый инструмент решает свою задачу.»

### Вопрос: «Что делать если тест flaky?»

**Ответ:**
> «Сначала анализирую — почему шаткий? Если race condition — добавляю retry или wait. Если внешний сервер нестабилен — заменяю на mock. Если тест сам по себе ненадёжный — переписываю с явными ожиданиями. Не просто перезапускаю CI в надежде, что в следующий раз пройдёт.»

### Вопрос: «CI падает — что смотришь первым делом?»

**Ответ:**
> «1. Логи упавшего job — самая нижняя красная строка. 2. Артефакты (screenshots, junit xml) если настроены. 3. Повторяется ли локально (`npm test`). 4. Если локально проходит — ищу delta: может версия Node, env переменная, или кэш npm.»
